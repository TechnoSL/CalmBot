...
module.exports = {
    execute(message, args, bot){
        message.channel.send("Please wait while we apply your roles **this can take some time**...").then(m => {
             fetch(`https://api.hypixel.net/player?key=${KEY}&player=${args[0]}&name=${args[0]}`).then(result => result.json()).then(({ player }) => {

            let socialMedia;

            if(message.guild.id != "673356418367356948") { //Testing Purposes, lets any commands in my test server bypass this
                try{
            ...
            if(player.packageRank != null)
            rank = player.packageRank;

            if(player.newPackageRank != null) //VIP-MVP+
            rank = player.newPackageRank
            
            if(player.monthlyPackageRank != "NONE" && player.monthlyPackageRank != null)//MVP++
            rank = player.monthlyPackageRank
            
            if(player.rank != null)//HELPER+
            rank = player.rank
